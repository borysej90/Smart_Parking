...
class ParkingLotsTestCase(APITestCase):
    ...
    def test_detail_put(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, 'Status code is not 200.')
        self.assertEqual(response.data['coordinates'], new_lot['coordinates'],
                         'Updated "coordinates" field is not equal to the requested one.')
        self.assertEqual(response.data['is_occupied'], new_lot['is_occupied'],
                         'Updated "is_occupied" field is not equal to the requested ones.')
        self.assertEqual(response.data['parking_site_id'], new_lot['parking_site_id'],
    ...
    def test_detail_delete(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, 'Status code is not 200.')
        self.assertEqual(deleted_lot.status_code, status.HTTP_404_NOT_FOUND, 'Parking lot wasn\'t deleted.')
